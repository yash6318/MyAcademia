...
from MainApp.models import Teachers, TeacherDetails, Classes, Students, Attendance, Results
...
def my_class(request):
    ...
    paginator = Paginator(all_studs, 10)  # Show 10 students per page
...
def attendance(request):
    if request.method == 'POST':
        ...
        if month == 'january':
            stud.january = attendance
        elif month == 'february':
            stud.february = attendance
        elif month == 'march':
            stud.march = attendance
        elif month == 'april':
            stud.april = attendance
        elif month == 'may':
            stud.may = attendance
        elif month == 'june':
            stud.june = attendance
        elif month == 'july':
            stud.july = attendance
        elif month == 'august':
            stud.august = attendance
        elif month == 'september':
            stud.september = attendance
        elif month == 'october':
            stud.october = attendance
        elif month == 'november':
            stud.november = attendance
        elif month == 'december':
            stud.december = attendance
    ...
    # Create a list of tuples for pagination
    zipped_list = list(zip(all_studs, attendance_of_stud))

    # Implement pagination
    paginator = Paginator(zipped_list, 10)  # Show 10 students per page
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
...
def marks(request):
    if request.method == 'POST':
        ...
        new_marks = int(request.POST.get('new_marks'))
        
        studd = Results.objects.get(enrollno=enrollno)

        if sub == 'sub1': 
            studd.marksinsub1 = new_marks
        elif sub == 'sub2': 
            studd.marksinsub2 = new_marks
        elif sub == 'sub3': 
            studd.marksinsub3 = new_marks
        elif sub == 'sub4': 
            studd.marksinsub4 = new_marks
        elif sub == 'sub5': 
            studd.marksinsub5 = new_marks
        elif sub == 'sub6': 
            studd.marksinsub6 = new_marks
        elif sub == 'sub7': 
            studd.marksinsub7 = new_marks
        print(studd.__dict__)
        studd.save()
    ...
    # Create a list of tuples for pagination
    zipped_list = list(zip(all_studs, marks_of_stud))
